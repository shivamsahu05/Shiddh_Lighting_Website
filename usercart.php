...
$query = "SELECT * FROM orders WHERE CustomerName = ? ORDER BY ordernum DESC";
...
if (!$result) {
    ...
    $phoneQuery = "SELECT Mobile_Num, ShopName FROM orders WHERE CustomerName = ? LIMIT 1";
    ...
    if ($phoneRow) {
        echo "<p class='phone'>Number: " . $phoneRow['Mobile_Num'] . "</p>";
        echo "<p class='phone'>Shop Name: " . $phoneRow['ShopName'] . "</p>";
    }
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<td>" . $date . "</td>";
        echo "<td style='font-weight: bold; color: black;'>" . $row['ordernum'] . "</td>";
        // Use a form to send data with POST
        echo "<td>
            <form action='usercartdetails.php' method='POST'>
                <input type='hidden' name='ordernum' value='" . $row['ordernum'] . "'>
                <input type='hidden' name='mobile' value='" . htmlspecialchars($phoneRow['Mobile_Num']) . "'>
